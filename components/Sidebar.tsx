...
const Sidebar: React.FC<SidebarProps> = ({ playlists, tags, filter, onFilterChange, onCreatePlaylist, onDeletePlaylist, isCollapsed, smartPlaylists }) => {
    ...
    return (
        <aside className={`bg-gray-800 flex flex-col h-full border-r border-gray-700 shadow-lg w-full overflow-hidden transition-all duration-300 ease-in-out ${isCollapsed ? 'p-0' : 'p-4'}`}>
            <div className={`flex-1 flex flex-col overflow-hidden transition-opacity duration-200 ${isCollapsed ? 'opacity-0' : 'opacity-100'}`}>
